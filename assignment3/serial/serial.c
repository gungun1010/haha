...
/*
  This is the serial program for CPSC424/524 Assignment #3.
...
int main(int argc, char **argv) {
  ...
  for (ts=0; ts<K; ts++) {
    ...
    for (thisbody=0; thisbody<N; thisbody++) {
      ...
      vavgx = vx[thisbody] + dtBy2*ax; // Compute average x velocity of thisbody
      vavgy = vy[thisbody] + dtBy2*ay; // Compute average y velocity of thisbody
      vavgz = vz[thisbody] + dtBy2*az; // Compute average z velocity of thisbody
